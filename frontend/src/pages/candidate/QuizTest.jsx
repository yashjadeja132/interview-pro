...
export default function QuizTest({ streams }) {
    ...
    useEffect(() => {
        if (!storedData) { return console.error("No candidate data found. Please login again.") }
    ...
    useEffect(() => {
        ...
        allTracks.forEach(track => track.addEventListener("ended", handleTrackEnded));
        return () => allTracks.forEach(track => track.removeEventListener("ended", handleTrackEnded));
    }, [streams])
    useEffect(() => { if (candidateData?.positionId) fetchQuestions() }, [candidateData])
    ...
    const handleSubmit = async (auto = false) => {
        if (!auto && Object.keys(answers).length !== questions.length) {
            return console.log("Please answer all questions!");
        }
        ...
        try {
            ...
            const response = await axios.post(`http://localhost:5000/api/test`, formdata, {
    ...
    const saveProgressToBackend = async (updatedAnswers = answers, updatedIndex = currentQuestionIndex + 1) => {
        ...
        try {
            ...
            await axios.post("http://localhost:5000/api/test-progress/save", progressPayload);
            ...
            <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center p-4">
                <Card className="w-full max-w-md border-0 shadow-xl bg-white/90 backdrop-blur-sm">
                    <CardContent className="p-8 text-center">
                        ...
                        <h3 className="text-lg font-semibold text-gray-900 mb-2">Loading Candidate Data</h3>
            ...
            <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center p-4">
                <Card className="w-full max-w-md border-0 shadow-xl bg-white/90 backdrop-blur-sm">
                    <CardContent className="p-8 text-center">
                        ...
                        <p className="text-gray-600">Preparing your assessment questions...</p>
                    </CardContent>
                </Card>
            </div>
        );
            ...
            <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center p-4">
                <Card className="w-full max-w-lg border-0 shadow-xl bg-white/90 backdrop-blur-sm">
                    <CardContent className="p-8 text-center">
                        ...
                        <p className="text-gray-600 mb-6">There are no questions available for this position at the moment.</p>
                        <Button 
                            onClick={() => navigate("/")}
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-gray-600">Complete all questions to finish your test</p>
                            </div>
                        </div>
                            ...
                            <CardContent>
                                ...
                                <div className="grid grid-cols-5 gap-2">
                                    {questions.map((q, index) => {
                                        ...
                                        const isVisited = visitedQuestions.includes(index) && !isAnswered;
                                        const notTouched = !visitedQuestions.includes(index) && !isAnswered;
                                        
                                        let className = "w-10 h-10 rounded-lg border-2 font-semibold text-sm transition-all duration-200 hover:scale-105 ";
                                        
                                        if (isCurrent) className += "bg-blue-500 border-blue-600 text-white shadow-lg";
                                        else if (isAnswered) className += "bg-green-500 border-green-600 text-white";
                                        else if (isVisited) className += "bg-red-500 border-red-600 text-white";
                                        else className += "bg-gray-100 border-gray-300 text-gray-600 hover:bg-gray-200";
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-3">
                                        <RadioGroup 
                                            ...
                                            onValueChange={(val) => handleChange(questions[currentQuestionIndex]._id, val)}
                                            className="space-y-3"
                                        >
                                        ...
                                        {answers[questions[currentQuestionIndex]._id] && (
                                            <div className="flex justify-center pt-4">
                                                <Button
                                                    onClick={() => handleClearAnswer(questions[currentQuestionIndex]._id)}
                                                    variant="outline"
